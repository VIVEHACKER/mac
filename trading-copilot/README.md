...
- `forecast` / `forecast-record` / `forecast-score` / `forecast-ledger`: next-release CPI/PPI nowcast (persistence/seasonal/AR/energy-bridge ensemble with honest expanding-window OOS skill), pre-registered to `out/forecast_ledger.jsonl` and scored against official prints once released
- `rate-forecast` / `rate-record` / `rate-score` / `rate-ledger`: next FOMC / Bank of Korea rate-decision forecast as {cut, hold, hike} probabilities from three auditable signals (short-rate spread vs policy rate, damped Taylor-rule pressure, hold-streak inertia). `rate-record` pre-registers to `out/rate_ledger.jsonl` only within `--horizon-days` (default 21) of the meeting; `rate-score` detects the announced decision from the policy-rate series and scores modal hit + multiclass Brier. Direction-only: bp size and emergency meetings are out of scope; the softmax constants are documented priors judged by the scored ledger

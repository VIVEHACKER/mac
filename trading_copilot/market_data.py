...
class YahooChartProvider:
    ...
    def snapshot(self, ticker: str) -> MarketSnapshot:
        ...
        source = f"{self.BASE_URL}/{quote(normalized)}"
        payload = self.fetch_json(source)
        meta = self._extract_meta(payload, normalized)

        price = required_number(meta, "regularMarketPrice", normalized)
        previous_close = optional_number(meta, "previousClose")
        if previous_close is None:
            previous_close = optional_number(meta, "chartPreviousClose")
        if previous_close is None:
            raise MarketDataError(f"{normalized}: missing previous close in Yahoo chart response")

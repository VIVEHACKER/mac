...
> ⚠️ **검증 상태 (2026-06): 이 명세는 "차트를 스펙대로 정확히 탐지하는" 알고리즘이며, 그 탐지가
> 수익 진입을 만든다는 검증은 통과하지 못했다.** Walk-forward forward-return 검증(16,681 시점, 크립토
> 4h + 주식 8년)에서 ENTER_NOW/SCALE_IN 신호는 예측 엣지가 없었다 — 크립토는 단기 역행, 주식은 중립~
> 장기 underperform, IC 전 horizon 음수. **진입 트리거로 실거래 사용 금지.** 시각화·맥락·VETO/회피
> 필터로만 사용하고, 검증된 AQR 라인 위 advisory 레이어로 둘 것. 상세: `docs/CHART_VALIDATION.md`.

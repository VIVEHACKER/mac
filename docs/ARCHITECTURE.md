...
│       ├── themes.py        # 매크로 스펙·US 슈퍼테마+하위산업·KR 테마 loader(validated CSV→catalog, ETF 폴백)·티커 칩
│       ├── fetch.py         # yfinance 배치(+재시도·심볼 폴백) / FRED(기존 fred_macro 재사용)
│       ├── panels.py        # 확장 패널 — 검증선정·forward-OOS원장·거시예측·KR수급(naver 추정) (fail-open)
│       ├── render.py        # 서버사이드 HTML 렌더 (pctPair 색 램프, 드릴다운/패널/딥링크)
│       └── build.py         # 카탈로그(US·KR 테마·OOS 마크) + yfinance(매크로/칩) + FRED(스프레드) + 패널 조립
│   # KR 테마 유니버스: data/kr_theme_universe.csv(큐레이션) → scripts/kr_universe_ingest.py
│   #   (pykrx 이름 대조 검증 + 카탈로그 적재) → data/kr_theme_universe.validated.csv (build 소비)

...
| **IDEAL 방어 오버레이** (SPY 200일선 cash 필터, `scripts/ideal_defensive_walkforward.py`) | 2026-06-14, 단일 사전선언 규칙(그리드 없음)·전환비용 10bps·동일 walk-forward+스트레스 게이트 | walk-forward: positive 86.7→73.3%·excess +8.15→+2.92%·Sharpe 1.41→1.28·MDD 19.19→26.16%(전부 악화); 스트레스: 2018Q4 −16.42→−19.56·COVID +35→+19·2022 +23→−4 | **NOT ADOPTED** — 사전선언 바 둘 다 FAIL. 200일선 필터가 휩쏘로 **크래시 방어조차 악화**시키고 불마켓 엣지 반토막. "명백한 개선"이 병보다 나쁜 약. 변종 추가탐색 안 함(통과까지 검색=데이터마이닝). 결론: 이 long-only 모멘텀은 표준 트렌드필터로 크래시-하드닝 불가 | `out/ideal-defensive-walkforward.md` |
...
| **이머징 샷건** (Biberion 다큐 → 소형주 바스켓 모멘텀+레짐게이트+ATR스톱, `scripts/shotgun_walkforward.py`, [STRATEGY_NOTES.md](STRATEGY_NOTES.md)) | 2026-06-18, **일봉 프록시·full-sample(롤링 OOS 윈도우 없음 — 다른 *_walkforward와 달리 in-sample concept screen)**·현재 S&P600 소형주 579종(**생존편향+UNPINNED**)·IWM 5/25 **크로스이벤트** 게이트·**다음봉 체결**·**true daily MTM**·동시성캡 진입시 적용·1261트레이드·생존편향 매칭 EW(일별리밸·가용종목) 통제벤치 | per-trade EV **+0.22%**·win 35%·페이오프 **2.01x**(샷건 비대칭 재현); 포트 CAGR +0.81%·Sharpe **0.17**·MDD −19.4% vs IWM 0.59·SPY 0.86·**EW소형주(생존편향통제) 0.84** | **REJECTED** — H1(트렌드 EV>0) PASS·**H2(레짐의존성) FAIL**(choppy≥trend, 다큐 핵심주장 불성립)·H3(동조성) directional+. **샷건 SHAPE(per-trade 비대칭)만 재현, 포트폴리오로는 IWM·SPY·생존편향 EW(0.84) 전부에 열위**(0.17, 대부분 현금이라 저위험·저수익). ⚠️**codex 적대적 리뷰 3라운드가 버그/룩어헤드 4건 적발, 매번 Sharpe 하락**: 2.41(동시성 보유일캡 아티팩트)→0.67(진입시 캡)→0.26(`ret/hold` 분배 룩어헤드 스무딩 제거)→0.17(크로스이벤트 진입+다음봉 체결). 최초 2.41은 전부 아티팩트=거짓양성. **PIT 검증 시도(2019-06 멤버십 동결, `out/shotgun/shotgun-pit2019.md`)**: 무료데이터 한계 확정 — 2019 멤버십 993종 중 **319종(32%) 상폐+yfinance 가격 전무**(JCP/CHK/AKRX 등), 무료론 survivorship-free 불가. 2019-동결 생존자(692) Sharpe **0.48**(H1+H2 PASS)도 IWM/SPY/EW(0.56/0.89/0.78) 전부 열위 + 상폐 tail 누락이라 진짜는 더 낮음. 자본 미배선. 진짜 PIT=유료(CRSP/Norgate~$65mo/Shardar~$50mo) | `scripts/shotgun_walkforward.py` → `out/shotgun/shotgun-walkforward.md`·`shotgun-pit2019.md` |

...
| **결합 IDEAL 80% + 저변동성 20%** (가중 사전선언, 그리드 금지) | 2026-06-11 | positive **100%(15/15)**, 평균 초과 **+6.50%/yr**, Sharpe **1.48**, worst MDD **16.61%** | **PASS** — 사전선언 3바 전부 충족: ①standalone ②Sharpe 1.48≥IDEAL 단독 1.41 ③worst MDD 16.61%≤19.19%. 트레이드: 초과 −1.65pp ↔ 일관성 13/15→15/15·꼬리 −2.6pp. 블렌드 수학 수치검증(공통월 기준 슬리브 사이 위치 확인, 단위테스트 5). **다음 게이트: PBO/fee-stress 통과 전 자금배분 금지** | `scripts/combined_ideal_lowvol_walkforward.py` → `out/combined-ideal-lowvol-walkforward.md` |
